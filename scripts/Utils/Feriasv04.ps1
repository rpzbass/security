...
function retornoFerias{
    ...
    foreach($list in $lista){
        ...
        $login = Get-AdUser -Filter {displayname -eq $list} | Select-Object -ExpandProperty SamAccountname
...
function  resetSenhaPadrao{
        ...
        Set-ADAccountPassword -Identity $login -Reset -NewPassword (ConvertTo-SecureString -AsPlainText "A123456@" -Force) # Resetar a senha
